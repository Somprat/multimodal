...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize SimplerEnv Bridge evaluation results.")
    parser.add_argument(
        "roots",
        nargs="*",
        help="Checkpoint parent roots. Each root should contain an eval_simpler/<checkpoint-name> directory.",
    )
    parser.add_argument("--dir-flag", default="eval_simpler", help="Evaluation directory name under each root.")
    parser.add_argument("--style", choices=("text", "md"), default="md", help="Output table style.")
...
if __name__ == "__main__":
    ...
    if not args.roots:
        raise SystemExit(
            "No roots provided. Example: python script/eval/bridge/extract_bridge_results.py "
            "./log/bridge/run_a ./log/bridge/run_b"
        )
    extract_and_print_success(args.roots, dir_flag=args.dir_flag, print_style=args.style)
