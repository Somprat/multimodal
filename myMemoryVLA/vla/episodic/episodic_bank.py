...
class EpisodicMemBank(nn.Module):
    def __init__(self, max_steps: int = 5,
        ...
        # CLIP supplies fixed retrieval keys; it is not one of the episodic
        # modules being adapted. Processor outputs are moved to its FSDP device.
    ...
    def train(self, mode: bool = True):
        super().train(mode)
        # Parent .train() calls must not enable dropout in frozen CLIP.
        self.clip_model.eval()
        return self

    def _to_clip_device(self, inputs):
        return inputs.to(next(self.clip_model.parameters()).device)
